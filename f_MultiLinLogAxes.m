        ...
        if opt.numAxes
            if ~isempty(opt.axesNumTxt{(j-1)*numColumns + n})
                ...
                if opt.external_labels
                    ...
                    tbh(tbh_index).Position(2) = h(axisPlotted+1).Position(2) + h(axisPlotted+1).Position(4) + opt.numAxesYoffset;% - tbh(tbh_index).Position(4) - 0.01*h(axisPlotted+1).Position(4);
